defmodule Camelot.Runtime.Runner.Swarm.ExecSessionTest do
  ...
  describe "container_replaced?/2" do
    test "delegates to AdoptPolicy so the reconciler can reach the check" do
      newer = elem(DateTime.from_iso8601("2026-07-14T12:47:00Z"), 1)
      older = elem(DateTime.from_iso8601("2026-07-14T10:44:00Z"), 1)

      assert ExecSession.container_replaced?(newer, older)
      refute ExecSession.container_replaced?(older, newer)
    end
  end
