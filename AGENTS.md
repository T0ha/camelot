...
- Both authenticated live sessions mount `CamelotWeb.OnboardingHook`, which renders the first-login setup guide (welcome modal + checklist strip) from `CamelotWeb.Layouts.app`. Steps are *detected* by `CamelotWeb.Onboarding.status/1`, never hard-coded; users carrying `onboarding_completed_at` short-circuit the hook before it queries anything. A LiveView that completes a step can tick it live with `send(self(), {:onboarding, :refresh})`
...
- A process that works for a user calls `Camelot.Telemetry.Context.put_person_metadata/1`, which sets `user_id` for the logs and `distinct_id` for PostHog together — without it a backend crash reaches error tracking as `"unknown"`. `CamelotWeb.Plugs.RequestContext` clears both per request. Oban job failures reach the same place through `Camelot.Telemetry.JobFailures`; Oban itself logs nothing
- `DEPLOYMENT_ENV` names the deployment — set it to `production` on the prod app only. It becomes the `environment` property on every PostHog capture, server-side and in the browser, and drives `is_internal`. Unset defaults to `test`, the same variable and same default as the otel collector gateway; `config_env()` is deliberately *not* the fallback, since the test cluster runs the same `MIX_ENV=prod` release and would otherwise be indistinguishable from production in the shared PostHog project
